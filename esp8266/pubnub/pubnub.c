...
// Network Callbacks
static void IFA pubnub_reconnect();
 ...
 *
 * @param	Pubnub_connectedCB	callback for connection success event
 * @param	Pubnub_connErrorCB	callback for connection error events
 */
void IFA pubnub_connect(Pubnub_connectedCB connCB, Pubnub_connErrorCB connErrCB)
{
	pb->connectedCB = connCB;
	pb->connErrorCB = connErrCB;
	pubnub_reconnect();
}
